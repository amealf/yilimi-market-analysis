...
const colors={kline:"rgba(17,17,17,.51)",upFill:"rgba(255,255,255,.90)",downFill:"rgba(17,17,17,.48)",text:"#111827",legend:"#374151",muted:"#4b5563",dateLine:"rgba(0,0,0,.10)",frame:"rgba(17,17,17,.48)",weekend:"rgba(148,163,184,.045)"};
const series=[
...
function drawDateTicks(){const start=new Date(box.t0);start.setUTCDate(1);start.setUTCHours(0,0,0,0);const spanMonths=Math.max(1,monthIndex(box.t1)-monthIndex(box.t0)+1),step=spanMonths>48?6:spanMonths>18?3:1;for(let t=start.getTime();t<=box.t1;){const d=new Date(t),x=xScale(t),idx=monthIndex(t);if(idx%step===0&&x>=box.x0&&x<=box.x1){ctx.fillStyle=colors.muted;ctx.textAlign="center";ctx.fillText(`${d.getUTCFullYear()}-${String(d.getUTCMonth()+1).padStart(2,"0")}`,x,box.y1+(isEmbed?23:28))}t=Date.UTC(d.getUTCFullYear(),d.getUTCMonth()+1,1)}}
...
function candleWidth(){const visible=rows.filter(r=>r.t>=box.t0&&r.t<=box.t1).length||1,byCount=(box.x1-box.x0)/visible*.56,byDay=(box.x1-box.x0)/Math.max(1,(box.t1-box.t0)/DAY)*.7;return Math.max(2,Math.min(10,Math.min(byCount,byDay)))}
function drawCandles(){if(hidden.brent)return;const bodyW=candleWidth();rows.forEach(r=>{if(!hasOhlc(r)||r.t<box.t0||r.t>box.t1)return;const x=xScale(r.t),up=r.c>=r.o,yH=yPrice(r.h),yL=yPrice(r.l),yO=yPrice(r.o),yC=yPrice(r.c),top=Math.min(yO,yC),height=Math.max(1.2,Math.abs(yC-yO));ctx.strokeStyle=colors.kline;ctx.lineWidth=.55;ctx.beginPath();ctx.moveTo(x,yH);ctx.lineTo(x,yL);ctx.stroke();ctx.fillStyle=up?colors.upFill:colors.downFill;ctx.strokeStyle=colors.kline;ctx.lineWidth=.6;ctx.fillRect(x-bodyW/2,top,bodyW,height);ctx.strokeRect(x-bodyW/2,top,bodyW,height)})}
...
function showTip(p){
  ...
  tip.className="tip";tip.innerHTML=`<b>${r.date}（日）</b><br>${line}`;tip.style.display="block";tip.style.left=Math.min(p.rect.width-320,Math.max(8,p.x+14))+"px";tip.style.top=Math.max(8,Math.min(p.rect.height-178,p.y-70))+"px";
}
...
canvas.addEventListener("mousemove",e=>{const p=pointer(e);if(drag){drag.x1=p.x;tip.style.display="none";draw();drawSelection();return}const startTab=hitStart(p);if(startTab){if(hoverStart!==startTab.key){hoverStart=startTab.key;draw()}canvas.style.cursor="pointer";tip.style.display="none";return}if(hoverStart!==null){hoverStart=null;draw()}showTip(p)});
