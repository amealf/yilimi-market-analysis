...
const series=[
  ...
  {key:"btcEtfFlow",label:"BTC ETF累计净流入",color:colors.btcEtfFlow,scale:"etf",width:1.05}
];
...
function yFor(item,v){if(item.scale==="ratio")return yRatio(v);if(item.scale==="rate")return yRate(v);if(item.scale==="etf")return yEtf(v);return ySupply(v)}
function gridLine(y){ctx.strokeStyle=colors.grid;ctx.lineWidth=.65;ctx.beginPath();ctx.moveTo(box.x0,y);ctx.lineTo(box.x1,y);ctx.stroke()}
...
function drawModeTabs(x,y){
  ...
  labels.forEach(([key],i)=>{
    const w=30,h=20,left=x+i*(w+6),active=priceMode===key,hovered=hoverMode===key;
    ...
    ctx.lineWidth=1.15;
    if(key==="line"){
      ctx.beginPath();ctx.moveTo(left+7,y+13);ctx.lineTo(left+13,y+8);ctx.lineTo(left+18,y+10);ctx.lineTo(left+23,y+5);ctx.stroke();
    }else{
      ctx.beginPath();ctx.moveTo(left+11,y+5);ctx.lineTo(left+11,y+15);ctx.moveTo(left+20,y+4);ctx.lineTo(left+20,y+16);ctx.stroke();
      ctx.fillRect(left+8,y+8,6,5);ctx.strokeRect(left+8,y+8,6,5);
      ctx.fillStyle=hovered||active?"rgba(17,17,17,.70)":"rgba(255,255,255,.88)";
      ctx.fillRect(left+17,y+7,6,7);ctx.strokeRect(left+17,y+7,6,7);
    }
  });
  ctx.lineWidth=1;
}
...
function draw(active,eventDate=null){
  ...
  box={x0,x1,y0,y1,t0,t1,ratioMin:Math.log(Math.max(ratioMin0*.75,.01)),ratioMax:Math.log(ratioMax0*1.18),supplyMin:Math.min(0,supplyMin0*1.1),supplyMax:Math.max(supplyMax0*1.1,1),rateMin:rateMin0-ratePad,rateMax:rateMax0+ratePad,etfMin:etfMin0-etfPad,etfMax:etfMax0+etfPad};
  ...
  const periodX=x1-154,controlY=compactHeader?titleY+18:titleY-15;
  drawLegend(x0,legendY,compactHeader?x1:periodX-84);drawModeTabs(periodX-76,controlY);drawPeriodTabs(periodX,controlY);
