...
def chart_meta(data: pd.DataFrame) -> dict:
    ...
    kospi_peak = kospi.loc[kospi["kospi_close"].idxmax()]
    ...
    return {
        ...
        "financingPeakKospi": nearest_value(financing_peak["date"], "kospi_close", 2),
        "kospiPeakDate": str(kospi_peak["date"]),
        "kospiPeak": round(float(kospi_peak["kospi_close"]), 2),
        "kospiPeakRatio": round(float(kospi_peak["kospi_ratio"]), 4),
        "kospiPeakFinancing": nearest_value(kospi_peak["date"], "credit_financing_trillion_krw", 6),
    }
